...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	speedValue := runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift
	weightmInKm := weight / mInKm
	durationminInH := duration * minInH
	return speedValue * weightmInKm * durationminInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	weightValue := walkingCaloriesWeightMultiplier * weight
	speedMeterSec := meanSpeed(action, duration) / 3.6
	speedHeight := math.Pow(speedMeterSec, 2) / height
	weightValueMultiplier := walkingSpeedHeightMultiplier * weight
	timeValue := duration * minInH
	return (weightValue + speedHeight*weightValueMultiplier) * timeValue
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	speedValueSwim := swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift
	weigtValueSwim := swimmingCaloriesWeightMultiplier * weight * duration
	return speedValueSwim * weigtValueSwim
